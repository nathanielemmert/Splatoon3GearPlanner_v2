...
<script lang="ts">
    ...
    type KeysOfType<O, T> = {[K in keyof O]: O[K] extends T ? K : never}[keyof O & string];

    // props
    type ItemRowObject = $$Generic;
    export let items:ItemRowObject[];
    ...
    export let itemHeight:HeightPx|undefined = undefined; // the height of each row


    // read-only, but visible to consumers via bind:start resp. bind:end
    export let start:number = 0 // the index of the first visible item
    export let end:number = 0 // the index of the last visible item

    export let rowsPerItem = 2;
    export let fixedColWidth=true;

    type TheadElement = HTMLTableSectionElement;
    type TbodyElement = HTMLTableSectionElement;
    type HeightPx = number;

    // local state
    let averageHeight:HeightPx;
    let bottom = 0;
    let contents: TbodyElement;
    let headHeight:HeightPx = 0;
    let footHeight:HeightPx = 0;
    let heightMap:HeightPx[] = [];
    let mounted:boolean;
    let rows: HTMLCollection;
    let thead:TheadElement;
    let top:HeightPx = 0;
    let viewport:HTMLTableElement;
    let viewportHeight = 0;
    let visible: {index: number, data: ItemRowObject}[];
    ...
    async function refreshHeightMap(items:ItemRowObject[], viewportHeight:HeightPx, itemHeight:HeightPx|undefined) {
        const scrollTop:HeightPx = viewport.scrollTop;
        await tick() // wait until the DOM is up to date
        await repaint()//TODO: remove
        let contentHeight:HeightPx = top - (scrollTop - headHeight)
        let i:number = start;
        while (
            contentHeight < viewportHeight - headHeight &&
            i < items.length
        ) {
            let row = rows[i - start+(fixedColWidth?1:0)]// rows[i-start:i-start+rowsPerItem]
            if (!row) {
                end = i + 1
                await tick() // render the newly visible row
                await repaint()//TODO: remove
                row = rows[i - start+(fixedColWidth?1:0)]
            }
    ...
    function getComputedPxAmount(elem:Element, pseudoElem:string|null, property:string) {
        const compStyle = getComputedStyle(elem, pseudoElem)
        return parseFloat(compStyle[property])
    }

    async function handleScroll() {
        rows = contents.children
        console.log("ROWS",rows)
        const isStartOverflow:boolean = sortedItems.length < start
        const rowBottomBorder = getComputedPxAmount(
            rows[1+(fixedColWidth?1:0)],
            null,
            'border-bottom-width'
        )
        ...
        const actualBorderCollapsedWidth =
        ...
        for (let v = (fixedColWidth?1:0); v < rows.length; v += 1) {
            heightMap[start + v-(fixedColWidth?1:0)] =
...
<svelte-virtual-table-viewport>
    <table
            ...
            class="{CLASSNAME_TABLE} {className} table"
            bind:this={viewport}
            bind:offsetHeight={viewportHeight}
            on:scroll={handleScroll}
            role="table"
            style="table-layout: auto;height: {height}; --bw-svt-p-top: {top}px; --bw-svt-p-bottom: {bottom}px; --bw-svt-head-height: {headHeight}px; --bw-svt-foot-height: {footHeight}px; --bw-svt-avg-row-height: {averageHeight}px"
    >
        <thead
                class="thead"
                role="rowgroup"
